...
const updateDocument = async(name,_id)=>{
try{
    const result = await table.updateOne({name} ,
   { $set : {name : "Arslan"}
    })
    console.log("result is",result);
    // const result2 = await table.updateMany({age :age1},{$set : {age: 24}})
    // console.log("updated Age",result2);

       const result2 = await table.findByIdAndUpdate({_id},{$set : {age: 27}},{new : true})
...
updateDocument("Kami", '63e51a6cc1f94147029fd856');